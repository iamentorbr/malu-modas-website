...
export default function LoginVendedora({ onLogin }: { onLogin: (nome: string) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error || !data.user) {
      setErro("E-mail ou senha incorretos. Fale com a Malu.")
      setLoading(false)
      return
    }
    ...
    const nome =
      data.user.user_metadata?.nome_vendedora ||
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-xs uppercase tracking-[.2em] text-muted-foreground">Acesso da Vendedora</p>
        </div>
        ...
        <p className="mt-4 text-center text-xs text-muted-foreground">
          Precisa de acesso? Fale com a <span className="font-semibold text-foreground">Malu</span>.
        </p>
      </div>
    </div>
  )
